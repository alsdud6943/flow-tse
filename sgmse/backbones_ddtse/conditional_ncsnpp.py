...
class ConditionalNCSNpp(nn.Module):
    ...
    def forward(self, x, ref, mix_condition, time_cond):
        ...
        Args:
            ...
            ref (torch.Tensor): Precomputed speaker embedding tensor (B, emb_dim)
        ...
        """
        condition = ref  # (B, 1, emb_dim)
        # print("ref shape", ref.shape)  # (B, 1, emb_dim)

        # process condition
        if self.spk_encoder_backbone: # False by default
            # For backward compatibility, still use the original method if spk_encoder_backbone is True
            # Convert waveform to spectrogram for the original encoder
            ref_input = ref.unsqueeze(1)  # (B, T) -> (B, 1, T)
            # Note: This would need proper STFT conversion if spk_encoder_backbone is used
            # print("Condition before spk_encoder", condition.shape) #[batchsize, 1, L]
            condition = self.speaker_encoder_model(ref_input)
        ...
        x = torch.cat([x, mix_condition], dim=1)  # (B, 2, F, T) -> (B, 4, F, T)
        ...
        h = self.output_layer(h) # (B, C=2, F, T)
        h = torch.permute(h, (0, 2, 3, 1)).contiguous() # (B, 2, F, T) -> (B, F, T, 2)
        h = torch.view_as_complex(h)[:,None, :, :] # (B, F, T) - complex tensor
        return h
